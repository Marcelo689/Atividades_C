...
int main(void){
	...
	int quantidade,i,numeros=0,tamanho=1;
	
	string mesaS,quantidadeS,codigoS;
	
	
	while(tamanho != numeros){
...
//	while(codigo != 999){// Pergunta pelos pedidos enquanto codigo for diferente de 999; 
	...
	if(numeros == tamanho){
		...
		// reinicia o loop
		printf("Voce digitou uma codigo invalido tente novamente !!!\n");
		continue;
	}
	...
	if(quantidade == 0){
		printf("Nao eh possivel pedir numero zero desse item \n");
		continue;
	}
	}
